...
__all__ = "OBB", "Classify", "Detect", "Pose", "RTDETRDecoder", "Segment", "YOLOEDetect", "YOLOESegment", "v10Detect", "TGDetect"
...
class TGDetect(nn.Module):
    """YOLO Detect head with Dynamic Task Fusion for detection models."""
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
        self.task_gate = nn.ModuleList(
            nn.Sequential(
                nn.Conv2d(x, 2, 1),  # predict two weights: for regression and classification
                nn.Sigmoid()
            ) for x in ch
        )
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            weights = self.task_gate[i](x[i])  # shape: (B, 2, H, W)

            w_reg = weights[:, 0:1, :, :]
            w_cls = weights[:, 1:2, :, :]

            reg_out = reg_out * w_reg
            cls_out = cls_out * w_cls
            out = torch.cat([reg_out, cls_out], 1)
            outputs.append(out)
